...
from sources import prompt_template
...
def download_final_links(**kwargs):
    ti = kwargs.get('ti')
    all_urls = ti.xcom_pull(task_ids='fetch_urls')
    final_links = []
...
def extract_text_from_links(**kwargs):
    ti = kwargs.get('ti')
    final_links = ti.xcom_pull(task_ids='download_final_links')
    texts = []
...
def extract_data_with_gpt(**kwargs):
    load_dotenv(dotenv_path=os.path.join(os.path.dirname(__file__), "../.env"))
    openai.api_key = os.getenv('OPENAI_API_KEY')
    model="gpt-4"
    texts = kwargs.get('ti').xcom_pull(task_ids='extract_text_from_links')
    results = []
    for item in texts:
